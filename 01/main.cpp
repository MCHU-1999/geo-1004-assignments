...
double triangle_max_z(const Kernel::Triangle_3& triangle) {
  return std::max({triangle.vertex(0).z(), triangle.vertex(1).z(), triangle.vertex(2).z()});
}

double triangle_min_z(const Kernel::Triangle_3& triangle) {
  return std::min({triangle.vertex(0).z(), triangle.vertex(1).z(), triangle.vertex(2).z()});
}
...
int main(int argc, const char *argv[]) {
  // Example: parse --input <file> and --output <file>
  ...
  for (auto &face: input_faces) {
    ...
    double this_max = triangle_max_z(face.triangle);
    double this_min = triangle_min_z(face.triangle);
    if (blocks[face.block_id].max < this_max) {
      blocks[face.block_id].max = this_max;
    }
    if (blocks[face.block_id].min > this_min) {
      blocks[face.block_id].min = this_min;
    }
  }
